...
export default class Dogs extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      dogs: [],
  ...
  getLastViewed = index => {
    fetch(`https://api.thedogapi.com/v1/images/${index}`)
  ...
  render() {
    const { dogs, lastViewedImages } = this.state;
    return (
      <div style={{ textAlign: 'center' }} className="mt-3 mb-5">
        <h1 className="mb-3">Hello, I am a dog!</h1>
        {dogs.map(item => (
          <div key={item.id}>
            <img
